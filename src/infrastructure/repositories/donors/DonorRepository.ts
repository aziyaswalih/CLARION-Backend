import DonorModel, {
  DonorDocument,
} from "../../../infrastructure/database/models/DonorModel";
...
export class DonorRepository implements IDonorRepository {
  ...
  async updateDonor(
    ...
    return await DonorModel.findOneAndUpdate({ donorId: id }, updateData, {
      new: true,
    });
  }
  ...
  async getDonationsByDonorId(donorId: string): Promise<any[]> {
    const donations = await DonationModel.find({ donorId: donorId })
      .populate("storyId")
      .sort({ date: -1 });
    if (!donations) return [];
    return donations;
