import VolunteerModel, {
  VolunteerDocument,
} from "../../../infrastructure/database/models/VolunteerModel";
...
export class VolunteerRepository implements IVolunteerRepository {
  ...
  async getVolunteerById(id: string): Promise<Volunteer | null> {
    return await VolunteerModel.findOne({ volunteerId: id }).populate(
      "volunteerId"
    );
  }
  ...
  async updateVolunteer(
    id: string,
    updateData: Partial<Volunteer>
  ): Promise<Volunteer | null> {
    return await VolunteerModel.findOneAndUpdate(
      { volunteerId: id },
      updateData,
      { new: true }
    );
