...
import {
  IMessage,
  MessageModel,
} from "./infrastructure/database/models/MessageModel";
...
export const socket_Connection = () => {
  io.on("connection", (socket) => {
    ...
    socket.on("register", async (userType: string, userId: string) => {
      console.log(
        `User registered: ${userType}, ID: ${userId}, Socket: ${socket.id}`
      );
    ...
    socket.on(
      ...
      async ({
        ...
        console.log(
          `Received 'deleteMessage' for ID: ${messageId} from socket: ${socket.id}`
        );

        try {
          // Find the message to get sender information
          const messageToDelete = await MessageModel.findOne({
            messageId: messageId,
          });
          ...
          const userIdFromSocket = [...activeUsers.entries()].find(
          ...
          if (
            ...
            console.warn(
              `Unauthorized delete attempt for message ${messageId} by user ${userIdFromSocket}. Sender was ${messageToDelete.sender}.`
            );
            // Optional: Emit an error back to the sender
            socket.emit("deleteError", {
              messageId: messageId,
          ...
          const participantSocketIds = participants
            ...
            ); // Filter out undefined and the socket that initiated the delete (they already did optimistic update)

          console.log(
            `Broadcasting 'messageDeleted' for ID ${messageId} to sockets:`,
    ...
    socket.on("markAsRead", async ({ sender, receiver }) => {
      ...
      try {
        ...
        if (senderSocketId) {
          ...
          console.log(
            `📢 Sent 'messagesRead' event to sender: ${senderSocketId}`
          );
        }
      } catch (error) {
    ...
    socket.on("callData", ({ senderId, receiverId, callType }) => {
      ...
      if (usersideId) {
        io.to(usersideId).emit("callDetails", {
          senderId,
          receiverId,
          callType,
        });
    ...
    socket.on("acceptCall", ({ roomId, employeeId }) => {
      console.log(
        `✅ Call Accepted in room: ${roomId}, Employee ID: ${employeeId}`
      );
    ...
    socket.on(
      ...
      ({
        ...
        senderId: string; // The original caller
        receiverId: string; // The user who missed the call (whose client emitted this)
        roomId: string;
        reason: string; // Should be 'timeout'
      }) => {
        console.log(
          `🚫 Received 'call_missed': Room ${roomId}, Caller: ${senderId}, Missed by: ${receiverId}, Reason: ${reason}`
        );
        ...
        if (callerSocketId) {
          ...
          io.to(callerSocketId).emit("call_not_answered", {
          ...
          console.log(
            `📢 Notified caller ${senderId} (Socket: ${callerSocketId}) that call to ${receiverId} in room ${roomId} was not answered (Reason: ${reason}).`
          );
        } else {
          console.log(
    ...
    socket.on(
      ...
      async ({
        ...
        try {
          ...
          const savedmessage: IMessage = await new MessageModel({
            sender,
            receiver,
            message,
            userType,
          ...
          if (recipient) {
            ...
            console.log(
              `Message sent to ${receiver} (Socket: ${recipient.socketId})`
            );
          } else {
