...
export class BeneficiaryUserController {
  ...
  async submitDetails(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.log(
        decodedToken,
        beneficiaryDetails,
        "decoded token beneficiary details"
      );
      const user = decodedToken.id;

      const newBeneficiary = await this.submitBeneficiaryDetailsUseCase.execute(
        { ...beneficiaryDetails, user: Object(user) }
      );
  ...
  async updateBeneficiary(req: Request, res: Response): Promise<void> {
    try {
      ...
      const updatedBeneficiary = await this.updateBeneficiaryUseCase.execute(
        id,
        {
          ...updates,
